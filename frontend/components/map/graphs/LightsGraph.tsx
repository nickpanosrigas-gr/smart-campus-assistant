...
export default function LightsGraph({ artifact }: { artifact: any }) {
  if (!artifact?.series || artifact.series.length === 0) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center text-[#A3B8B2]/60 p-8 text-center bg-black/20 rounded-3xl border border-white/5">
        <Sun size={36} className="mb-3 text-[#14C89B]/40 animate-pulse" />
        <p className="text-sm font-semibold">No Illumination Data Recorded</p>
      </div>
    );
  }
  ...
  // Extract light sensor column names dynamically
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-[#14C89B]/10 shrink-0">
        <div>
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Sun size={20} className="text-[#14C89B]" />
            Room {artifact.room_id?.toUpperCase()} Ambient Lights
            <span className="text-xs font-mono uppercase bg-[#14C89B]/20 text-[#14C89B] px-2.5 py-0.5 rounded-full border border-[#14C89B]/30">
              {artifact.timeframe}
            </span>
          </h3>
          <p className="text-xs text-[#A3B8B2]/70 mt-0.5">Discrete illumination intensity (0 = Dark, 5 = Very Sunny)</p>
        </div>
      </div>
      ...
      <div className="flex-1 w-full pt-6 min-h-[220px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={formattedData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <YAxis stroke="#A3B8B2" fontSize={11} tickLine={false} axisLine={false} domain={[0, 5]} ticks={[0, 1, 2, 3, 4, 5]} />
            <Tooltip
              contentStyle={{ backgroundColor: "#0A0A0A", borderColor: "#0A664F", borderRadius: "16px", color: "#fff", fontSize: "12px" }}
              formatter={(value: any, name: any) => [`Level ${value}`, name]}
              labelStyle={{ color: "#14C89B", fontWeight: "bold", marginBottom: "4px" }}
            />
            {sensorKeys.map((key, i) => (
              <Area key={key} type="stepAfter" dataKey={key} name={key} stroke={i === 0 ? "#f59e0b" : "#14C89B"} strokeWidth={3} fillOpacity={1} fill="url(#lightGrad)" />
            ))}
          </AreaChart>
        </ResponsiveContainer>
      </div>
